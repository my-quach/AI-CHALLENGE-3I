...
cnn_model = load_model('cnn_model (1).keras')
...
def capture_from_webcam():
    ...
    # Đọc nhiều frame để lấy frame mới nhất
    for _ in range(5):
        ret, frame = cap.read()

    if ret:
        webcam_active = False  # Tắt webcam stream
...
def show_qr_code_from_file(file_path):
    ...
    #def on_qr_window_close():
        #account_frame["points"] += 20
       # messagebox.showinfo("Thành công", "Thanh toán thành công. Bạn nhận được 20 điểm thưởng!")
        #if points_label.winfo_exists():
         #   points_label.config(text=f"⭐ Điểm thưởng: {account_frame['points']}")
        #else:
         #   print("points_label không tồn tại.")
        #qr_window.withdraw()

    #qr_window.protocol("WM_DELETE_WINDOW", on_qr_window_close)
        
# Nếu có label hiển thị điểm thưởng thì update
